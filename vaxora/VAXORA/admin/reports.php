        ...
        <div class="card chart-card h-100">
            ...
            <div class="card-body">
                <div class="row g-2 mb-4">
                    ...
                    <div class="col-6"><div class="p-3 rounded-3 text-center" style="background:#F4F0E6;border:1px solid #D4C9B0;"><div class="fw-bold h4 mb-0 text-violet"><?= $stats['total_appt'] ?></div><div class="small text-muted">Total</div></div></div>
                </div>
                <div>
                    ...
                    <div class="progress" style="height:8px;border-radius:4px;">
                        <div class="progress-bar" style="width:<?= round($stats['hospitals']/10*100) ?>%;background:linear-gradient(90deg,#4A6120,#8B5E3C);border-radius:4px;"></div>
                    </div>
                </div>
            </div>
        </div>
...
<div class="row g-4">
        ...
        <div class="card">
            ...
            <div class="card-body p-0">
                ...
                <table class="table mb-0">
                    ...
                    <tr>
                        <td><span class="badge" style="background:<?= ['#C8A84B','#9ca3af','#cd7f32','#4A6120','#8B5E3C','#4A7C59'][$i] ?? '#9ca3af' ?>;"><?= $i+1 ?></span></td>
    ...
    <div class="col-md-6">
        <div class="card">
            <div class="card-header"><i class="fas fa-vial me-2" style="color:#4A6120;"></i>Most Used Vaccines</div>
            <div class="card-body p-0">
                <?php if ($topVaccines): ?>
                <table class="table mb-0">
                    ...
                    <tr>
                        ...
                        <td class="text-end"><span class="badge" style="background:#F4F0E6;color:#4A6120;border:1px solid rgba(74,97,32,0.2);"><?= $v['cnt'] ?></span></td>
                    </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        datasets: [
            {
                ...
                borderColor: '#4A6120', backgroundColor: 'rgba(74,97,32,0.08)',
                fill: true, tension: 0.4, pointRadius: 4, pointBackgroundColor: '#4A6120'
            },
            {
                label: 'Appointments', data: <?= json_encode($monthlyAppts) ?>,
                borderColor: '#8B5E3C', backgroundColor: 'rgba(139,94,60,0.08)',
                fill: true, tension: 0.4, pointRadius: 4, pointBackgroundColor: '#8B5E3C'
            }
        ]
    },
    options: {
...
new Chart(document.getElementById('genderChart').getContext('2d'), {
    ...
    data: {
        ...
        datasets: [{ data: [<?= $genderData['male'] ?>, <?= $genderData['female'] ?>, <?= $genderData['other'] ?>],
            backgroundColor: ['#8B5E3C','#C0392B','#C8A84B'], borderWidth: 0 }]
    },
    options: {
        cutout: '65%',
...
new Chart(document.getElementById('vaccineChart').getContext('2d'), {
    ...
    data: {
        ...
        datasets: [{ label: 'Doses', data: vaccineCounts,
            backgroundColor: ['#4A6120','#8B5E3C','#4A7C59','#C8A84B','#C0392B','#627D2D','#A08030','#3D5020'],
