...
if (isset($_GET['delete'])) {
  ...
  $childId = (int)$_GET['delete'];

  // Check if child has appointments
  $stmt = $db->prepare('SELECT COUNT(*) FROM appointments WHERE child_id = ?');
  $stmt->execute([$childId]);
  $appointmentCount = $stmt->fetchColumn();

  if ($appointmentCount > 0) {

      setFlash('error', 'Cannot delete this child because vaccination appointments exist.');

  } else {

      $db->prepare('DELETE FROM children WHERE id=? AND parent_id=?')
         ->execute([$childId, $uid]);

      setFlash('success', 'Child removed.');
  }

  redirect(SITE_URL.'/parent/children.php');
}
