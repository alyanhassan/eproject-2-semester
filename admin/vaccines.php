...
if (isset($_GET['delete'])) {
  ...
  // Check if vaccine is used in appointments
  $check = $db->prepare("SELECT COUNT(*) FROM appointments WHERE vaccine_id=?");
  $check->execute([$id]);

  if ($check->fetchColumn() > 0) {

      setFlash('error', 'This vaccine has appointments and cannot be deleted.');

  } else {

      $db->prepare('DELETE FROM vaccines WHERE id=?')->execute([$id]);
      setFlash('success', 'Vaccine deleted.');

  }

  redirect(SITE_URL.'/admin/vaccines.php');
}
