...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .cert-page { box-shadow: none !important; border: 2px solid #4A6120 !important; }
        }
        .cert-border {
            border: 3px solid transparent;
            background: linear-gradient(white, white) padding-box,
                        linear-gradient(135deg, #4A6120, #8B5E3C, #C8A84B) border-box;
        }
    </style>
</head>
<body>
...
<div class="container py-5" style="max-width:820px;">
    <div class="cert-page cert-border" style="background:white;border-radius:24px;overflow:hidden;box-shadow:0 20px 60px rgba(91,33,182,0.15);">
        ...
        <!-- Certificate Header -->
        <div style="background:linear-gradient(135deg,#1C1408 0%,#3D2B10 50%,#4A6120 100%);padding:44px;text-align:center;color:white;position:relative;overflow:hidden;">
            <!-- Decorative circles -->
            ...
            <div style="position:relative;z-index:1;">
                ...
                <div style="margin-bottom:20px;">
                    <div style="display:inline-flex;align-items:center;gap:12px;background:rgba(255,255,255,0.1);padding:10px 24px;border-radius:50px;border:1px solid rgba(255,255,255,0.15);backdrop-filter:blur(8px);">
                        <i class="fas fa-syringe" style="font-size:1.2rem;color:#D8E8A8;"></i>
                ...
                <div style="font-size:0.75rem;letter-spacing:3px;color:#D8E8A8;text-transform:uppercase;margin-bottom:8px;">Official Document</div>
                <h1 style="font-size:2rem;font-weight:800;margin-bottom:6px;">Vaccination Certificate</h1>
                <p style="color:#C8BE9A;font-size:0.9rem;margin:0;">Issued by Vaxora E-Vaccination Management System, Pakistan</p>
        ...
        <div style="padding:44px;">
            <!-- Child Info -->
            <div style="background:#F4F0E6;border-radius:16px;padding:28px;margin-bottom:28px;border:1px solid rgba(74,97,32,0.15);">
                <div style="display:flex;align-items:center;gap:16px;margin-bottom:20px;">
                    <div style="width:64px;height:64px;background:linear-gradient(135deg,#627D2D,#4A6120);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.6rem;flex-shrink:0;">
                        <?= $child['gender']==='female' ? '👧' : '👦' ?>
                    </div>
                    <div>
                        <h4 style="font-weight:800;color:#2C1F0A;margin-bottom:4px;"><?= htmlspecialchars($child['full_name']) ?></h4>
                        <div style="color:#4A6120;font-size:0.82rem;font-weight:600;">Child ID: <?= htmlspecialchars($child['unique_child_id'] ?? 'VAX-'.$child['child_id']) ?></div>
                    </div>
                </div>
            ...
            <div style="margin-bottom:28px;">
                <h5 style="font-weight:700;color:#2C1F0A;margin-bottom:16px;display:flex;align-items:center;gap:8px;">
                    <span style="width:28px;height:28px;background:linear-gradient(135deg,#627D2D,#4A6120);border-radius:8px;display:inline-flex;align-items:center;justify-content:center;">
                ...
                <div style="border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
                    <table style="width:100%;border-collapse:collapse;font-size:0.85rem;">
                        <thead>
                            <tr style="background:linear-gradient(135deg,#1C1408,#3D2B10);color:white;">
                        ...
                        <tr style="background:<?= $i%2===0 ? 'white' : '#fafafa' ?>;border-top:1px solid #f3f4f6;">
                            ...
                            <td style="padding:12px 16px;font-weight:600;color:#2C1F0A;"><?= htmlspecialchars($r['vaccine_name']) ?></td>
            ...
            <!-- Summary Badge -->
            <?php if ($records): ?>
            <div style="background:linear-gradient(135deg,rgba(74,97,32,0.06),rgba(139,94,60,0.06));border:1px solid rgba(74,97,32,0.18);border-radius:12px;padding:20px;display:flex;align-items:center;gap:16px;margin-bottom:28px;">
                ...
                <div>
                    <div style="font-weight:700;color:#2C1F0A;">Total: <?= count($records) ?> Vaccination<?= count($records)!=1?'s':'' ?> on Record</div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;margin-bottom:28px;">
                <div style="text-align:center;">
                    <div style="height:50px;margin-bottom:8px;display:flex;align-items:flex-end;justify-content:center;">
                        <div style="font-family:cursive;font-size:1.4rem;color:#4A6120;opacity:0.7;">Dr. Ahmed Raza</div>
                    </div>
                    <div style="border-top:1px solid #e5e7eb;padding-top:8px;">
                        <div style="font-size:0.78rem;font-weight:700;color:#2C1F0A;">Dr. Ahmed Raza</div>
                        <div style="font-size:0.72rem;color:#9ca3af;">Chief Medical Officer</div>
                    </div>
                </div>
                <div style="text-align:center;">
                    <div style="height:50px;margin-bottom:8px;display:flex;align-items:flex-end;justify-content:center;">
                        <div style="width:70px;height:70px;background:linear-gradient(135deg,#4A6120,#8B5E3C);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto;">
                            <i class="fas fa-shield-alt" style="color:white;font-size:1.5rem;"></i>
                        </div>
                    </div>
                    <div style="border-top:1px solid #e5e7eb;padding-top:8px;">
                        <div style="font-size:0.78rem;font-weight:700;color:#2C1F0A;">Vaxora Official Seal</div>
                        <div style="font-size:0.72rem;color:#9ca3af;">E-Vaccination System, Pakistan</div>
                    </div>
                </div>
                <div style="text-align:center;">
                    <div style="height:50px;margin-bottom:8px;display:flex;align-items:flex-end;justify-content:center;">
                        <div style="font-family:cursive;font-size:1.4rem;color:#8B5E3C;opacity:0.7;">Dr. Sara Khan</div>
                    </div>
                    <div style="border-top:1px solid #e5e7eb;padding-top:8px;">
                        <div style="font-size:0.78rem;font-weight:700;color:#2C1F0A;">Dr. Sara Khan</div>
            ...
            <!-- Verification Footer -->
            <div style="background:#1C1408;border-radius:12px;padding:16px 20px;text-align:center;">
                <div style="font-size:0.75rem;color:#A89870;margin-bottom:4px;">Verify this certificate at</div>
                <div style="font-size:0.85rem;color:#D8E8A8;font-weight:600;">www.vaxora.pk/verify · Certificate: <?= $cert_no ?></div>
                <div style="font-size:0.72rem;color:#7A6A48;margin-top:4px;">This is a digitally generated certificate. Valid without physical signature. Issued: <?= $issued_date ?></div>
            </div>
        </div>
    </div>
</div>
